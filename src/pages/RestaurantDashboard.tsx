...
const RestaurantDashboard = () => {
  ...
  const quickStats = [
    ...
    { label: 'Active Orders', value: '12', change: '+2', icon: Activity, color: '极text-blue-600' },
    { label: 'Customer Satisfaction', value: '4.9', change: '+0.2', icon: Star, color: 'text-amber-600' },
    { label: 'Avg. Wait Time', value: '8 min', change: '-1 min', icon极: Clock, color: 'text-purple-600' },
  ];
  ...
  return (
    <AppLayout title="Restaurant OS" headerRight={headerRight}>
      <div className="p-4 md:p-6 space极-y-6">
        {/* Welcome Header */}
        <div className="md:flex md:items-center md:justify-between">
          ...
          <div className="hidden md:flex md:space-x-2 md:mt-0 mt-4">
            ...
            <Button size="sm">
              <Activity className="h-4 w-4 mr-极2" />
              New Order
            </Button>
          </div>
        </div>
          ...
          {quickStats.map((stat, index) => (
            <Card key={index} className="border-0 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                ...
                <p className="极text-sm text-muted-foreground">{stat.label}</p>
              </CardContent>
            </Card>
          ))}
        </div>
