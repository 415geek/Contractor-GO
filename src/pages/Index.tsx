    ...
    <AppLayout>
      <div className="flex flex-col space-y-6 md:space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-slate-900">早安, 杰克</h1>
            <p className="text-sm text-slate-500">这是您今天的业务概览。</p>
          </div>
          <Button className="hidden md:flex bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-6 shadow-lg shadow-indigo-100">
            ...
            <Card key={stat.name} className="border-none shadow-sm hover:shadow-md transition-shadow duration-200 rounded-2xl overflow-hidden">
              <CardContent className="p-4 md:p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary" className="hidden sm:flex bg-slate-100 text-slate-600 border-none text-[10px]">
                    +12% <ArrowUpRight className="ml-1 h-3 w-3" />
                  </Badge>
                </div>
                <div className="mt-3 md:mt-4">
                  <p className="text-xs md:text-sm font-medium text-slate-500">{stat.name}</p>
        ...
        <div className="space-y-4">
          ...
          {/* Desktop Table */}
          <Card className="hidden md:block border-none shadow-sm rounded-2xl overflow-hidden">
          ...
          <div className="md:hidden space-y-3">
            {recentJobs.map((job) => (
              <Card key={job.id} className="border-none shadow-sm rounded-2xl overflow-hidden active:bg-slate-50 transition-colors">
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900">{job.client}</h4>
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs">
                    <div className="text-slate-500">
                      <span className="font-medium text-slate-900">{job.amount}</span> • {job.date}
                    </div>
